    ...
    },
    {
        ...
        "http": [{
            ...
            "matchers": [
                {"type": "status", "status": [200]},
                {"type": "regex", "part": "body", "condition": "or",
                 "regex": [r"""(?i)['"]?(api[_-]?key|client[_-]?secret|jwt[_-]?secret|db[_-]?password|access[_-]?key[_-]?id)['"]?\s*:\s*['"](?!test|xxxx|changeme|sample|demo)[^'"]{8,}['"]"""]},
            ],
        }],
    },
    {
            ...
            "matchers": [
                ...
                {"type": "regex", "part": "body", "condition": "or",
                 "regex": [r"-----BEGIN (OPENSSH|RSA|DSA|EC) PRIVATE KEY-----",
                           ...
                           r"(?sm)^\s*Host\s+\S+.*?^\s*IdentityFile\s+\S+",
                           r"(?sm)^\s*Host\s+\S+.*?^\s*StrictHostKeyChecking\s+\S+",
                           r"(?sm)^\s*Host\s+\S+.*?^\s*ProxyCommand\s+\S+",
                           r"(?sm)^\s*Host\s+\S+.*?^\s*(User|Port)\s+\S+"]},
            ],
        }],
    },
    {
